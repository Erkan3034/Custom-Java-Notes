...
public class BookValidationApp {
    ...
    public static void validateBookPlate(String bookPlate) throws ValidationException { //uygulamanın hata fırlatma durumu olduğu için özel hata sınıfımızı kullanmak için throws kullanıyorm
        ...
        String pattern = "^[A-Z]{2,4}-\\d{4,6}$"; //a-z aralığında 2-4 arası karakter + 4-6 adet digit yani rakam içermeli ....
    ...
    public static void validateEmail(String mail) throws ValidationException{
    	String patternMail = "^[a-zA-Z0-9.+%-]+@[a-zA-Z0-9.-]+\\.[a-zA-Z0-9]{2,}$";
    	
    	if(!Pattern.matches(patternMail,mail)) {
    		throw new ValidationException("Geçersiz mail Adresi! Örnek:example@example.com");
    	}
    	
    	Pattern pt = Pattern.compile(patternMail);
    	Matcher mc = pt.matcher(mail);
    	
    	if (!mc.find()) {
    		throw new ValidationException("Geçersiz mail Adresi!");
